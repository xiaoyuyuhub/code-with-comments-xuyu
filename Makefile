...
HOSTCFLAGS	:= -g -Wall -O0 -D_FILE_OFFSET_BITS=64

GDB		:= cgdb

CC		:= $(GCCPREFIX)gcc
CFLAGS	:= -fno-builtin -fno-PIC -Wall -ggdb3 -m32 -gstabs -nostdinc $(DEFS)
...
# 在当前路径下创建一个 128MB 的文件 bin/sfs.img ,用于文件系统
...
QEMUOPTS = -drive file=$(UCOREIMG),format=raw,index=0,media=disk -drive file=$(SWAPIMG),format=raw,media=disk,cache=writeback -drive file=$(SFSIMG),format=raw,media=disk,cache=writeback 
...
# 本地调试方法: 
# 		开启 1 个终端, make debug-qemu
# 		开启另一个终端, make debug-gdb, 即可分开gdb 输出和 qemu 输出,避免花屏,输出更加清晰

# 远程调试方法:
#		1) make debug-qemu
#		2) clion 中配置, 192.168.1.17:1234, 必须配置 map, 最好复制二进制文件到本地,并在 clion 中指定

debug-qemu: $(UCOREIMG) $(SWAPIMG) $(SFSIMG)
...
debug-clion: $(UCOREIMG) $(SWAPIMG) $(SFSIMG)
	qemu-system-i386 -S -s -hda ./bin/ucore.img -monitor stdio  -drive file=./bin/swap.img
