 ...
 * 从文件的offset处读取 count 个字节到虚拟地址 va. 扇区号=(offset / SECTSIZE) + 1. kenerl 位于 1 号.
 * 封装了对于 va 的处理
 * */
static void
...
bootmain(void) {
   ...
   // 把每个 program 加载到其期待被加载到的虚拟地址位置.
    ...
    for (; ph < eph; ph ++) {
        ...
        //把ucore img kernel内核文件读取到虚拟内存上面去，ph->p_memsz则是内存起始地址
        //& 0xFFFFFF 实际上就是把取后面6位，0x100000,现在还没有进入entry文件，还没有建立虚拟地址，现在还是使用实地址
